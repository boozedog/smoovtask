...
- **st mediates all writes.** Agents and humans interact through the `st` CLI. No direct file edits to system files.
- **Opinionated workflow.** Fixed stages, enforced transitions, strict rules. Humans can override; agents cannot.
- **Observable by default.** Every hook fires into st. The JSONL event log is a complete audit trail. TUI and web UI stream it live.
...
~/obsidian/smoovtask/                   # Obsidian vault (default, configurable)
└── tickets/
    ├── 2026-02-25T10:00-st_a7Kx2m.md   # date prefix for chronological sorting
    ├── 2026-02-25T10:30-st_b3Yz9q.md
    └── ...
```

The Obsidian vault contains **only markdown** — things humans read. The JSONL event log and config live in `~/.smoovtask/` — machine data that powers the TUI, web UI, and access control.
...
```jsonl
{"ts":"2026-02-25T10:00:00Z","event":"ticket.created","ticket":"st_a7Kx2m","project":"api-server","actor":"human","session":null,"data":{"title":"Add rate limiting to API","priority":"P2"}}
{"ts":"2026-02-25T10:01:00Z","event":"ticket.assigned","ticket":"st_a7Kx2m","project":"api-server","actor":"st","session":null,"data":{"assignee":"agent-backend-01"}}
{"ts":"2026-02-25T10:02:00Z","event":"status.in-progress","ticket":"st_a7Kx2m","project":"api-server","actor":"agent-backend-01","session":"a1b2c3","data":{}}
{"ts":"2026-02-25T10:05:32Z","event":"hook.post-tool","ticket":"st_a7Kx2m","project":"api-server","actor":"agent-backend-01","session":"a1b2c3","data":{"tool":"Edit","file":"internal/middleware/ratelimit.go"}}
{"ts":"2026-02-25T11:30:00Z","event":"status.review","ticket":"st_a7Kx2m","project":"api-server","actor":"agent-backend-01","session":"a1b2c3","data":{"note":"Implemented token bucket algorithm"}}
{"ts":"2026-02-25T12:00:00Z","event":"status.rejected","ticket":"st_a7Kx2m","project":"api-server","actor":"agent-review-02","session":"d4e5f6","data":{"reason":"No context cancellation handling"}}
```
...
They are complementary. st writes both atomically on every operation.
...
When an agent requests to review a ticket (`st review st_xxxxxx`), st:
...
- When a dependency reaches DONE, st scans for dependents and auto-unblocks them (snaps back to prior status)
- Creates Obsidian links: `depends-on: [[st_b3Yz9q]]`

**Human holds:**
- A human can block any ticket with a freeform reason: `st hold st_a7Kx2m "waiting on API keys from vendor"`
- Only a human can release a hold: `st unhold st_a7Kx2m`

Both are stored as BLOCKED status. The JSONL event records the cause:

```jsonl
{"event":"status.blocked","ticket":"st_a7Kx2m","reason":"depends-on","ref":"st_b3Yz9q","prior_status":"OPEN"}
{"event":"status.blocked","ticket":"st_c1Dw4n","reason":"hold","message":"waiting on API keys","prior_status":"IN-PROGRESS"}
```
...
Format: `st_xxxxxx` where `xxxxxx` is 6 characters of **base62** (a-z, A-Z, 0-9).
...
[settings]
vault_path = "~/obsidian/smoovtask"  # default, configurable
...
**Registration:** `st init` in a project directory registers it. Creates a project entry in config.toml.

**Project detection:** When hooks fire or commands run, st determines the project from `$PWD`. If the current directory is inside a registered project path, events are tagged with that project. If not recognized, events go to an "untracked" bucket (no crash, no noise).
...
When a Claude session starts, the `SessionStart` hook fires and calls `st hook session-start`. st reads the session context from stdin JSON (which includes `session_id`, `cwd`, `source`), detects the project from `cwd`, and returns an `additionalContext` response with a board summary filtered to the current project:
...
Pick a ticket with `st pick st_xxxxxx`, or use /st for guided workflow.
To work all 3, spin up an agent team and assign one ticket per teammate.
```

st has already decided whether to present OPEN or REVIEW work (see Priority Model). The agent gets one clear batch and one job type.
...
1. Agent runs `st pick st_a7Kx2m` → ticket moves to IN-PROGRESS, assigned to this session
2. Agent does the work, runs `st note "..."` to document progress
3. Agent runs `st status review` → ticket moves to REVIEW
...
Each worker runs `st pick st_xxxxxx` on their assigned ticket.
Each worker does their work independently.
Each worker runs `st status review` when done.
```
...
Each reviewer runs `st review st_xxxxxx` on their assigned ticket.
...
- The orchestrator can monitor progress via `st list --project api-server`
...
**How st decides what to present at session start:**

st does NOT show both OPEN and REVIEW. It makes the call for the agent:
...
st web [--port 8080]                 # Start web UI server
```

### Agent Workflow Commands

```
st pick [st_xxxxxx]                  # Pick up a ticket (assigns to current session)
st note "message"                    # Append a note to current ticket
st status <status>                   # Transition current ticket to new status
st review st_xxxxxx                  # Request to review a specific ticket
st context                           # Print current session context (used by hooks)
```

### Human Management Commands

```
st assign st_xxxxxx <agent-id>       # Manually assign a ticket
st hold st_xxxxxx "reason"           # Block a ticket with a human hold
st unhold st_xxxxxx                  # Release a human hold
st override st_xxxxxx <status>       # Force a status (human override)
st close st_xxxxxx                   # Mark done (human shortcut)
```

### Hook Commands

```
st hook session-start                # Called by SessionStart — returns board context
st hook pre-tool                     # Called by PreToolUse — logs tool call
st hook post-tool                    # Called by PostToolUse — logs tool result
st hook subagent-start               # Called by SubagentStart — injects ticket context
st hook subagent-stop                # Called by SubagentStop — logs completion
st hook task-completed               # Called by TaskCompleted — logs completion
st hook teammate-idle                # Called by TeammateIdle — logs idle state
st hook permission-request           # Called by PermissionRequest — auto-approve (plugin)
st hook stop                         # Called by Stop — logs session stop
st hook session-end                  # Called by SessionEnd — cleanup
```
...
- `st status review`, `st review`, `st submit` → all do the same thing
- `st status in-progress`, `st start`, `st begin` → all do the same thing
- Clear error messages with guidance: "Can't move to REVIEW — no notes since starting work. Run `st note` first."
- `st help` is context-aware: shows relevant commands based on current ticket status
...
All hooks receive JSON on stdin with common fields: `session_id`, `cwd`, `transcript_path`, `permission_mode`, `hook_event_name`, plus event-specific fields. st reads stdin, extracts context, and writes to the JSONL event log.
...
```json
{
  "hooks": {
    "SessionStart": [
      {
        "matcher": "startup",
        "hooks": [{ "type": "command", "command": "st hook session-start" }]
      }
    ],
    "PreToolUse": [
      {
        "matcher": "*",
        "hooks": [{ "type": "command", "command": "st hook pre-tool", "async": true }]
      }
    ],
    "PostToolUse": [
      {
        "matcher": "*",
        "hooks": [{ "type": "command", "command": "st hook post-tool", "async": true }]
      }
    ],
    "SubagentStart": [
      {
        "hooks": [{ "type": "command", "command": "st hook subagent-start" }]
      }
    ],
    "SubagentStop": [
      {
        "hooks": [{ "type": "command", "command": "st hook subagent-stop", "async": true }]
      }
    ],
    "TaskCompleted": [
      {
        "hooks": [{ "type": "command", "command": "st hook task-completed", "async": true }]
      }
    ],
    "TeammateIdle": [
      {
        "hooks": [{ "type": "command", "command": "st hook teammate-idle", "async": true }]
      }
    ],
    "Stop": [
      {
        "hooks": [{ "type": "command", "command": "st hook stop", "async": true }]
      }
    ],
    "PermissionRequest": [
      {
        "hooks": [{ "type": "command", "command": "st hook permission-request" }]
      }
    ],
    "SessionEnd": [
      {
        "hooks": [{ "type": "command", "command": "st hook session-end", "async": true }]
      }
    ]
  }
}
...
| Hook | Blocking? | What st does |
|------|-----------|-------------|
| `session-start` | **Yes** (returns context) | Detects project from `cwd`. Returns `additionalContext` with board summary. Injects st workflow instructions into the session. |
| `pre-tool` | No (async) | Logs tool call to JSONL. |
| `post-tool` | No (async) | Logs tool result to JSONL. |
| `subagent-start` | **Yes** (returns context) | Logs subagent spawn. Returns `additionalContext` to inject st workflow instructions into the subagent (which ticket to work on, commands to use). |
| `subagent-stop` | No (async) | Logs subagent completion. |
| `task-completed` | No (async) | Logs task completion to JSONL. Does not block — ticket completion is decided by reviewers via `st review`, not by the task system. |
...
**Key design choice:** Most hooks are `async: true` (non-blocking). Only `session-start` and `subagent-start` are synchronous because they need to inject context. Ticket completion is enforced by reviewers via `st review`, not by hooks. This keeps st from slowing down the agent.

### SubagentStart Context Injection

This is how st gets workflow instructions into subagents spawned by an orchestrator. When an agent team is spun up:

1. Orchestrator spawns a teammate via Claude Code's Task tool
2. `SubagentStart` hook fires → `st hook subagent-start` runs
3. st reads the subagent's context, checks if the orchestrator has assigned it a ticket
4. st returns `additionalContext` with the ticket details and workflow commands

This means teammates automatically know which ticket they're working on and what st commands to use — the orchestrator doesn't have to explain the st workflow in every task prompt.
...
- **Go** net/http server, embedded in the `st` binary
...
```
┌─ api-server ─────────────────────────────────────────────┐
│ st_a7Kx2m  Add rate limiting       IN-PROGRESS  P2  ●●● │
│ st_c1Dw4n  Fix CORS headers        REVIEW       P3  ●●○ │
│ st_e5Fg8h  Update OpenAPI spec     BACKLOG      P4  ●○○ │
├─ smoovtask ─────────────────────────────────────────────┤
│ st_b3Yz9q  Implement TUI board     IN-PROGRESS  P2  ●●● │
│ st_d2Ex7p  Add SSE endpoint        OPEN         P3  ●●○ │
└──────────────────────────────────────────────────────────┘
```
...
│ BACKLOG    │ OPEN       │ IN-PROGRESS │ REVIEW     │ DONE       │
│            │            │             │            │            │
│ st_e5Fg8h │ st_d2Ex7p  │ st_a7Kx2m  │ st_c1Dw4n │            │
│ Update Op… │ Add SSE e… │ Add rate l… │ Fix CORS … │            │
│ api-server │ smoovtask │ api-server  │ api-server │            │
│            │            │             │            │            │
│            │            │ st_b3Yz9q  │            │            │
│            │            │ Implement … │            │            │
│            │            │ smoovtask  │            │            │
...
st has an internal plugin system that lets hook events trigger additional behaviors. Plugins are configured in `config.toml` and receive events from the JSONL stream. This is NOT a Claude Code plugin — it's st's own extension mechanism.

### Why Plugins

The core hook handlers (log to JSONL, inject context, enforce workflow) are built into st. But there are behaviors that are user-specific, environment-specific, or experimental — things that don't belong in the core but should be easy to wire up:

- **Auto-approval:** Automatically approve Claude Code permission prompts for trusted operations during st-managed work
...
name = "audio-alerts"
enabled = true
command = "st-plugin-audio"      # executable on PATH, or absolute path
...
name = "auto-approve"
enabled = true
command = "st-plugin-approve"
events = ["hook.permission-request"]

[plugins.config]
# Auto-approve Bash when working on an st ticket (other tools are safe by default)
allow_bash = true
# Only when the agent has an active st ticket assigned
require_active_ticket = true

[[plugins]]
name = "slack-notify"
enabled = false
command = "st-plugin-slack"
...
```
st event occurs → st writes to JSONL → st fans out to matching plugins

Plugin stdin:  {"ts":"...","event":"status.review","ticket":"st_a7Kx2m",...}
Plugin stdout: (optional JSON response, plugin-specific)
Plugin stderr: (logged by st for debugging)
```
...
The auto-approve plugin would hook into Claude Code's `PermissionRequest` hook (not st's plugin system directly — it would need to be wired as a Claude Code hook). When an agent has an active st ticket, the plugin auto-approves trusted tool calls:

1. `PermissionRequest` hook fires → `st hook permission-request` runs
2. st checks: does this session have an active ticket?
3. st checks: is the tool in the allow list?
...
Claude Code has a built-in task system (TaskCreate, TaskUpdate, TaskList, TaskGet) used for coordinating work within agent teams. smoovtask tickets and Claude Code tasks are **different layers** that work together:

| | Claude Code Tasks | smoovtask Tickets |
|--|--|--|
...
| **Created by** | Team lead via TaskCreate | Human or agent via `st new` |
| **Visible to** | Teammates in that session | Everyone, forever (Obsidian + web UI) |

### How They Interact

When an orchestrator agent picks up a batch of smoovtask tickets and creates a team:

1. **Orchestrator reads st tickets** → `st list --project X --status open`
2. **Orchestrator creates Claude Code tasks** for each teammate → uses TaskCreate to assign "Work on st_a7Kx2m"
3. **Each teammate starts** → `SubagentStart` hook fires, st injects ticket context
4. **Teammate runs `st pick st_a7Kx2m`** → st moves ticket to IN-PROGRESS
5. **Teammate does the work** → PreToolUse/PostToolUse hooks log activity to JSONL
6. **Teammate runs `st status review`** → st moves ticket to REVIEW
7. **Teammate marks Claude Code task complete** → `TaskCompleted` hook fires, st logs it to JSONL
8. **Session ends** → Claude Code tasks vanish, but st tickets persist with full history

The `TaskCompleted` hook logs the event but does not block. Ticket completion is decided by reviewers via `st review`, not by the task system. The two systems have different lifecycles and st does not interfere with Claude Code's task management.

### What st Does NOT Do

- st does NOT replace Claude Code's task system — it sits above it
- st does NOT create Claude Code tasks — the orchestrator agent does that natively
- st does NOT try to sync state between the two systems — they have different lifecycles
- Agents use Claude Code tasks for intra-session coordination and st tickets for persistent workflow
...
- [x] **Batch type selection:** st decides (not the agent). Presents one batch type based on priority scoring.
...
- [x] **st new priority default:** Defaults to P3 (Normal) when no priority specified.
- [x] **Plugin execution model:** Per-event invocation (stateless). MD and JSONL files are the state — no daemon, no long-running process. `st` is a pure CLI: read files, do work, write files, exit. Only `st web` and `st board` are long-lived (optional UIs that tail files, own no state).
...
- [x] **Ticket file names:** Flat directory with date-prefixed filenames: `2026-02-25T10:00-st_xxxxxx.md`. Creation timestamp gives chronological sorting in Obsidian and `ls`. Ticket ID is still the canonical identifier.
- [x] **Config format:** TOML. `BurntSushi/toml` in Go.
- [x] **Event log retention:** Keep JSONL forever for v1. Daily rotation keeps files manageable. Add `st gc` later if needed.
...
- [x] **Auto-approve safety:** Focused on Bash (the only dangerous tool). Other tools (Edit, Write, Read, etc.) are safe by default. `require_active_ticket` guard ensures agent is on an st ticket.
- [x] **Audio alerts cross-platform:** Config-driven via `player` and `tts_command` in TOML. macOS: `afplay`/`say`. Linux/NixOS: `paplay`/`espeak`. TTS announces ticket transitions ("Ticket st_a7Kx2m ready for review"). Example scripts ship in repo.
...
- **Observability:** Webhook plugin that POSTs event payloads to configurable endpoints. Enables integration with external dashboards, alerting systems, or custom tooling without modifying st core.

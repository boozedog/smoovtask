...
func BuildPrompt(tk *ticket.Ticket, runID, repoRoot string) string {
	...
	fmt.Fprintf(&b, "- To log progress: write your note to `%s-note.md` in the current directory using the Write tool, then run `st note --file %s-note.md --ticket %s --run-id %s` (the file is deleted after reading)\n", tk.ID, tk.ID, tk.ID, runID)
